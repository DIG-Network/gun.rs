/// Test: Chain.map() - Map with nested objects (two clients)
/// 
...
#[tokio::main]
async fn main() {
    println!("Test: Chain.map() - Map with nested objects (two clients)");
    ...
    let timestamp = std::time::SystemTime::now()
        ...
        .unwrap()
        .as_millis();
    let test_key = format!("map_nested_{}", timestamp);
    ...
    // Test: Client2 maps over nested properties
    println!("\n--- Test: Client2 mapping over nested properties ---");
    ...
    if props.len() >= 2 {
        println!("✓ Client2: Map nested - Success (found {} properties)", props.len());
        success_count += 1;
    } else {
        println!("✗ Client2: Map nested - Failed (found {} properties)", props.len());
        fail_count += 1;
    }
