/// Test: Chain.back() - Navigate back multiple levels (two clients)
/// 
...
#[tokio::main]
async fn main() {
    println!("Test: Chain.back() - Navigate back multiple levels (two clients)");
    ...
    // Test: Navigate back multiple levels - both clients can test (local operation)
    println!("\n--- Test: Navigate back multiple levels ---");
    let chain1 = client1.get("level1").get("level2").get("level3").get("level4");
    
    // Back 2 levels
    match chain1.back(Some(2)) {
        Some(_) => {
            println!("✓ Client1: Back 2 levels - Success");
            success_count += 1;
        }
        None => {
            println!("✗ Client1: Back 2 levels - Returned None");
            fail_count += 1;
        }
    }
    
    // Back 3 levels
    match chain1.back(Some(3)) {
        Some(_) => {
            println!("✓ Client1: Back 3 levels - Success");
            success_count += 1;
        }
        None => {
            println!("✗ Client1: Back 3 levels - Returned None");
            fail_count += 1;
        }
    }
    
    // Also test on client2
    let chain2 = client2.get("level1").get("level2").get("level3").get("level4");
    match chain2.back(Some(2)) {
        Some(_) => println!("✓ Client2: Back 2 levels - Success"),
        None => println!("✗ Client2: Back 2 levels - Returned None"),
    }
