/// Test: Chain.back() - Navigate back one level (two clients)
/// 
...
#[tokio::main]
async fn main() {
    println!("Test: Chain.back() - Navigate back one level (two clients)");
    ...
    // Test: Navigate back one level - both clients can test (local operation)
    println!("\n--- Test: Navigate back one level ---");
    let chain1 = client1.get("level1").get("level2").get("level3");
    
    match chain1.back(Some(1)) {
        Some(_back_chain) => {
            println!("✓ Client1: Back one level - Success");
            success_count += 1;
        }
        None => {
            println!("✗ Client1: Back one level - Returned None");
            fail_count += 1;
        }
    }
    ...
    match chain2.back(Some(1)) {
        Some(_back_chain) => {
            println!("✓ Client2: Back one level - Success");
        }
        None => {
            println!("✗ Client2: Back one level - Returned None");
        }
    }
