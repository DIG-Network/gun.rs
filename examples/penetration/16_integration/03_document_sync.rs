/// Test: Document synchronization (two clients)
/// 
...
#[tokio::main]
async fn main() {
    println!("Test: Document synchronization (two clients)");
    ...
    let timestamp = std::time::SystemTime::now()
        ...
        .unwrap()
        .as_millis();
    let test_key = format!("doc1_{}", timestamp);
    
    let mut success_count = 0;
    let mut fail_count = 0;
    
    println!("\n--- Test: Client1 document synchronization ---");
    match client1.get("documents").get(&test_key).put(json!({
        "title": "Test Document",
        "content": "This is test content",
        "version": 1
    })).await {
        Ok(_) => {
            println!("✓ Client1: Document synchronization - Document created");
            tokio::time::sleep(Duration::from_millis(2000)).await;
            success_count += 1;
        }
        Err(e) => {
            println!("✗ Client1: Document synchronization - Failed - {}", e);
            fail_count += 1;
        }
    }
