/// Test: Game state synchronization (two clients)
/// 
...
#[tokio::main]
async fn main() {
    println!("Test: Game state synchronization (two clients)");
    ...
    let timestamp = std::time::SystemTime::now()
        ...
        .unwrap()
        .as_millis();
    let test_key = format!("state_{}", timestamp);
    
    let mut success_count = 0;
    let mut fail_count = 0;
    
    println!("\n--- Test: Client1 game state synchronization ---");
    match client1.get("game").get(&test_key).put(json!({
        "players": [
            {"id": 1, "x": 10, "y": 20},
            {"id": 2, "x": 30, "y": 40}
        ],
        ...
        Ok(_) => {
            println!("✓ Client1: Game state synchronization - State created");
            tokio::time::sleep(Duration::from_millis(2000)).await;
            success_count += 1;
        }
        Err(e) => {
            println!("✗ Client1: Game state synchronization - Failed - {}", e);
            fail_count += 1;
        }
    }
