/// Test: Null handling (two clients)
/// 
...
#[tokio::main]
async fn main() {
    println!("Test: Null handling (two clients)");
    ...
    let timestamp = std::time::SystemTime::now()
        ...
        .unwrap()
        .as_millis();
    let test_key = format!("null_{}", timestamp);
    
    let mut success_count = 0;
    let mut fail_count = 0;
    
    // Test: Client1 null value
    println!("\n--- Test: Client1 null value ---");
    match client1.get("test").get(&test_key).put(json!(null)).await {
        Ok(_) => {
            println!("✓ Client1: Null value - Success");
            tokio::time::sleep(Duration::from_millis(1500)).await;
            success_count += 1;
        }
        Err(e) => {
            println!("✗ Client1: Null value - Failed - {}", e);
            fail_count += 1;
        }
    }
