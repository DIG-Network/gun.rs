/// Test: Rapid operations (two clients)
/// 
...
#[tokio::main]
async fn main() {
    println!("Test: Rapid operations (two clients)");
    ...
    println!("\n--- Test: Client1 rapid operations ---");
    let mut handles = vec![];
    for i in 0..100 {
        let client1_clone = client1.clone();
        let key = format!("rapid{}_{}", i, timestamp);
    ...
    if rapid_success >= 90 {
        println!("✓ Client1: Rapid operations - {}/100 succeeded", rapid_success);
        success_count += 1;
    } else {
        println!("✗ Client1: Rapid operations - Only {}/100 succeeded", rapid_success);
        fail_count += 1;
    }
