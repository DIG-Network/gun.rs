...
impl WebSocketClient {
    ...
    pub async fn connect(&self, url: &str) -> GunResult<()> {
        ...
        let public_url = ensure_public_url(&url_str)?;
        ...
        loop {
            match Self::connect_once(&public_url, core.clone(), mesh.clone()).await {
                Ok(_) => {
                    // Connection successful, wait a bit for handshake to complete
                ...
                Err(e) => {
                    ...
                    if retry_count >= max_retries {
                        return Err(crate::error::GunError::Network(format!("Max reconnection attempts reached for {}: {}", public_url, e)));
                    }
                    
                    // Exponential backoff: wait = base_wait * 2^retry_count (capped at 2 seconds)
                    let wait_time = base_wait * (1 << retry_count.min(4));
                    eprintln!("Connection failed for {}, retrying in {:?}... (attempt {})", public_url, wait_time, retry_count);
    ...
    async fn connect_once(url: &str, core: Arc<GunCore>, mesh: Arc<Mesh>) -> GunResult<()> {
        ...
        let url = Url::parse(&ws_url)
            .map_err(|e| crate::error::GunError::Network(format!("Invalid URL {}: {}", ws_url, e)))?;
        ...
        let (ws_stream, response) = connect_async(url.clone()).await
            .map_err(|e| crate::error::GunError::Network(format!("WebSocket connection failed to {}: {}", ws_url, e)))?;
        
        // Verify connection was successful (status 101 Switching Protocols)
        if response.status() != 101 {
            return Err(crate::error::GunError::Network(format!("WebSocket handshake failed with status: {}", response.status())));
        }
        ...
        // Set sender in mesh BEFORE adding peer (so messages can be sent immediately)
        mesh.set_peer_sender(&peer_id, tx.clone()).await?;
        
        // Add peer to mesh (this registers the peer for message routing)
